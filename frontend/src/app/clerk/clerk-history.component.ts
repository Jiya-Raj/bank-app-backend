  ...
  <div class="card" style="background:#000;border:1px solid #666;color:#fff;">
    <div class="card-body">
      ...
      <form class="row g-2 mb-3" [formGroup]="form" (ngSubmit)="load()">
        <div class="col-md-6"><input class="form-control" style="background:#000;color:#fff;border:1px solid #666;" formControlName="accountNumber" placeholder="Account number"></div>
      ...
      <p *ngIf="loaded && transactions.length===0">No transactions available.</p>
    </div>
  </div>
  `
})
export class ClerkHistoryComponent {
  ...
  load(): void {
    const account = this.form.getRawValue().accountNumber ?? '';
    this.txService.getHistory(account).subscribe((res) => {
      this.loaded = true;
